...
"""One-dimensional convolution via im2col and matrix multiplication.

Small supported CUDA shapes may use Triton; all other shapes use the explicit
Torch fallback.
"""
...
class Conv1d:
    ...
    def __init__(
        ...
        self.use_triton = (
            TRITON_AVAILABLE
            and self.col_size_padded <= self.MAX_TILE_DIM
        ...
        if self.use_triton and (
            self.col_size_padded != self.col_size or self.out_channels_padded != out_channels
        ):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if out_length <= 0:
            raise ValueError("kernel_size exceeds the padded input length")
