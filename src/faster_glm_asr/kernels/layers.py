...
"""Lightweight tensor layers with Torch/cuBLAS defaults and Triton options.

The classes are intentionally not ``torch.nn.Module`` instances. Weight
loading and device movement are explicit so benchmark paths can account for
them, while unsupported kernel shapes retain Torch fallbacks.
"""
...
class RMSNorm:
    ...
    def __init__(self, hidden_size: int, eps: float = 1e-6):
        ...
        self.use_triton = TRITON_AVAILABLE and _is_power_of_two(hidden_size)
...
def gelu(x: torch.Tensor) -> torch.Tensor:
    """Official-semantics exact GELU correctness path.

    The historical Triton kernel above implements the tanh approximation while
    the fixed GLM-ASR reference uses exact GELU.  Keep approximation as a future
    explicit benchmark variable instead of silently changing CUDA numerics.
    """
...
class Linear:
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        for name, value in {"in_features": in_features, "out_features": out_features}.items():
...
class Embedding:
    ...
    def __call__(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        torch._assert_async(
            torch.all((input_ids >= 0) & (input_ids < self.num_embeddings)),
            "input_ids contain an embedding index outside [0, num_embeddings)",
        )
...
class MLP:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        if self.use_gating and MLP.FUSED and TRITON_AVAILABLE and x.is_cuda:
...
class EncoderMLP:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        if EncoderMLP.FUSED and self.activation == "gelu" and TRITON_AVAILABLE and x.is_cuda:
            return self._forward_fused(x)
        return self._forward_standard(x)
